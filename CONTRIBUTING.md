...
| TRUST_PROXY                          | Configure proxy trust settings for proper IP detection. Defaults to `false` locally and one proxy hop on Railway. Values: `false`, a hop count, or Express proxy values (e.g., `loopback`, `linklocal`, `uniquelocal`, IP addresses). `true` is rejected because it trusts an arbitrary forwarded-IP chain. [Learn More](https://expressjs.com/en/guide/behind-proxies.html) | Boolean/String/Number                            | Auto                                |
