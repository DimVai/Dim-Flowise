...
Each server or queue-worker process initializes a shared catalog from the bundled `packages/components/models.json`, then loads the saved `model-catalog.json` if valid. The saved file is located in `DATABASE_PATH` when set, otherwise in `~/.flowise`; place this directory on persistent storage to retain the catalog across restarts and deployments. An explicit `MODEL_LIST_CONFIG_JSON` local file or HTTP(S) URL takes priority over the saved catalog. If that source cannot be loaded, the saved catalog remains active, or the bundled catalog if no valid saved file exists. The catalog is not fetched from upstream Flowise.

After base initialization, one request to `https://models.dev/api.json` refreshes only `chatOpenAI`, `chatAnthropic`, and `chatGoogleGenerativeAI`. The server waits for both base sources and this refresh (including fallback on failure) before it starts listening. The Models.dev and configured HTTP(S) requests each have a 10-second timeout. `DISABLE_DYNAMIC_MODELS=true` or `TRUE` disables the Models.dev startup request while still loading the base sources. The default is enabled.

Accepted lists completely replace the three dynamic lists in memory. A missing, invalid, or empty filtered provider list retains that provider's existing list; a failed request retains all existing lists. When at least one provider has an accepted list, the merged catalog is written to the saved JSON using a temporary file and rename. A read or write failure does not prevent startup; a write failure leaves the updated catalog in memory. Other integrations, embeddings, legacy LLM lists, regions, and group metadata are preserved. There is no polling or change to saved flows. Restart to reload the saved base or refresh the catalog; no manual refresh UI is provided.
...
| MODEL_LIST_CONFIG_JSON               | Optional local file path or HTTP(S) URL overriding the saved or bundled model catalog at startup. A failed load retains the saved catalog, then the bundled fallback. HTTP(S) requests have a 10-second timeout. See [Model catalog](#model-catalog). | String                                                |                                  |
| DISABLE_DYNAMIC_MODELS               | Disable the Models.dev startup refresh with `true` or `TRUE`; the saved or configured base still loads. See [Model catalog](#model-catalog).                                                                                                          | Boolean string                                        | false                            |
