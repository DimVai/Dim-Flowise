...
The About dialog displays the installed Flowise version and, on the next row, the Dim-Flowise version. The fork version is maintained in `forkVersion` in [`packages/server/package.json`](packages/server/package.json). Update that field in the corresponding version commit (currently `1.2`); commit messages do not update it automatically. The existing `version` field retains the upstream package version.
