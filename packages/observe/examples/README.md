...
| `VITE_API_TOKEN` | API key for authentication — get this from **Flowise UI → Settings → API Keys → Create New Key**. This is an API key, not a user session token. Stored permissions are not currently enforced by the community route permission middleware; do not rely on them to block operations such as deleting executions. See [Authentication](../../../CONFIGURATION.md#authentication). |
