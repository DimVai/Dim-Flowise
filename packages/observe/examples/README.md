...
| `VITE_API_TOKEN` | API key for authentication — get this from **Flowise UI → Settings → API Keys → Create New Key**. This is an API key, not a user session token. Ensure the key has the necessary permissions for the features you want to exercise, such as deleting executions. See [API keys and permissions](../../../CONFIGURATION.md#api-keys-and-permissions). |
