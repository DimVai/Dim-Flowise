 ...
 */
async function getAllApiKeysByOrganization(_organizationId: string): Promise<ApiKey[]> {
    const appServer = getRunningExpressApp()
    return appServer.AppDataSource.getRepository(ApiKey).findBy({ workspaceId: COMMUNITY_WORKSPACE_ID })
}
 ...
 */
const getAllApiKeys = async (user: CommunityAuthUser, page: number = -1, limit: number = -1) => {
    try {
        const appServer = getRunningExpressApp()
...
const createApiKey = async (user: CommunityAuthUser, keyName: string, permissions: string[]) => {
...
// Update api key
const updateApiKey = async (user: CommunityAuthUser, id: string, keyName: string, permissions: string[]) => {
