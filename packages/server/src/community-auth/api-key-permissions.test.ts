...
const mockFlows = [
    { id: 'chat', type: 'CHATFLOW', workspaceId: '0' },
    { id: 'agent', type: 'AGENTFLOW', workspaceId: '0' }
]
const mockKeyRepository: any = {
    ...
    save: jest.fn(async (key: ApiKey) => {
        if (!mockKeys.includes(key)) mockKeys.push(key)
        return key
    }),
    delete: jest.fn(async ({ id }: { id: string }) => {
        const i = mockKeys.findIndex((key) => key.id === id)
        if (i >= 0) mockKeys.splice(i, 1)
        return { affected: 1 }
    }),
    createQueryBuilder: () => {
    ...
    getRunningExpressApp: () => ({
        AppDataSource: {
            getRepository: (entity: any) =>
                entity === ApiKey ? mockKeyRepository : entity === UpsertHistory ? { find: async () => mockHistory } : mockFlowRepository
        }
    })
}))
...
const mounts: Record<string, string> = {
    ...
    'internal-chat-messages': 'internal-chatmessage',
    'internal-predictions': 'internal-prediction',
    vectors: 'vector',
    'openai-assistants-files': 'openai-assistants-file'
}
const app = express()
...
const envNames = ['FLOWISE_USERNAME', 'FLOWISE_PASSWORD', 'FLOWISE_SECRET', 'FLOWISE_JWΤ_DURATION'] as const
const originalEnv = Object.fromEntries(envNames.map((name) => [name, process.env[name]]))
const token = 'test-only-api-key'
const hash = generateSecretHash(token)
const key = (permissions: any = ['variables:view']): ApiKey =>
...
beforeEach(() => {
    process.env.FLOWISE_USERNAME = 'test-owner'
    process.env.FLOWISE_PASSWORD = 'test-password'
...
describe('API-key authorization through production middleware and routers', () => {
    ...
    it.each([
        ...
        ['post', 'document-store/upsert/store'],
        ['post', 'document-store/refresh/store'],
        ['post', 'document-store/vectorstore/insert'],
        ['post', 'document-store/generate-tool-desc/store'],
        ['delete', 'chatmessage/chat'],
        ['put', 'chatflows/chat'],
        ['delete', 'chatflows/chat'],
        ['post', 'chatflows'],
        ['post', 'openai-assistants-file/upload'],
        ['post', 'openai-assistants-vector-store/store'],
        ['post', 'vector/upsert/chat'],
        ['post', 'vector/internal-upsert/chat'],
        ['post', 'internal-prediction/chat'],
        ['post', 'openai-realtime/chat'],
        ['post', 'export-import/import'],
        ['post', 'export-import/export'],
        ['post', 'mcp-server/chat'],
        ['post', 'webhook-listener/chat/register']
    ...
    it.each([
        ['variables:create', 'post', 'variables'],
        ['variables:update', 'put', 'variables/id'],
        ['variables:delete', 'delete', 'variables/id'],
        ['documentStores:upsert-config', 'post', 'document-store/upsert/store'],
        ['documentStores:upsert-config', 'post', 'document-store/refresh/store'],
        ['chatflows:update', 'put', 'chatflows/chat'],
        ['chatflows:delete', 'delete', 'chatflows/chat'],
        ['agentflows:update', 'put', 'chatflows/agent'],
        ['assistants:update', 'put', 'assistants/id']
    ])('allows %s on %s %s', async (permission, method, path) => {
        mockKeys[0].permissions = [permission]
        await send(method, path).expect(204)
    })
    it.each([[], null, undefined, 'variables:view', ['*'], ['admin:all', 'workspace:export']])(
        'gives no management access to empty/malformed/legacy permissions %j',
        async (permissions) => {
    ...
    it('requires a real owner session for the internal header, even with a valid API key', async () => {
        ...
        await send('get', 'variables')
            .set('x-request-from', 'internal')
            .set('Cookie', COMMUNITY_AUTH_COOKIE + '=tampered')
            .expect(401)
    ...
    it('keeps permissions current across update, replacement and deletion', async () => {
        ...
        await send('put', 'apikey/key', { keyName: 'Updated', permissions: ['variables:update'] })
            .set('x-request-from', 'internal')
            .set('Cookie', cookie)
            .expect(200)
        await send('get', 'variables').expect(403)
        ...
        mockKeys[0].apiKey = 'replacement'
        mockKeys[0].apiSecret = generateSecretHash('replacement')
    ...
    it('rejects invalid keys, hashes and workspaces', async () => {
        ...
        mockKeys[0].workspaceId = 'other'
        await send('get', 'variables').expect(401)
        mockKeys[0].workspaceId = '0'
        mockKeys[0].apiSecret = generateSecretHash('wrong')
        await send('get', 'variables').expect(401)
    })
    it('serves the permission catalog to the owner and permits creating a flow-only key', async () => {
        const catalog = await request(app)
            .get('/api/v1/auth/permissions/API_KEY')
            .set('x-request-from', 'internal')
        ...
        await send('post', 'apikey', { keyName: 'Flow only', permissions: [] })
            .set('x-request-from', 'internal')
            .set('Cookie', ownerCookie())
            .expect(200)
    ...
    it.each(['workspace:import', 'admin:all', 'unknown:write', '*'])(
        'rejects assigning unsupported permission %s even for owner',
        async (permission) => {
            await send('post', 'apikey', { keyName: 'Bad', permissions: [permission] })
                .set('x-request-from', 'internal')
                .set('Cookie', ownerCookie())
                .expect(400)
        }
    ...
    it('flow execution keys remain independent of management permissions', async () => {
        ...
        mockKeys[0].permissions = ['chatflows:view']
        await request(app)
            .get('/api/v1/chatflows/apikey/' + token)
            .expect(204)
...
describe('public route boundaries', () => {
    it.each([
        ['GET', '/api/v1/feedback/chat'],
        ['GET', '/api/v1/leads/chat'],
        ['POST', '/api/v1/oauth2-credential/refresh/id'],
        ['DELETE', '/api/v1/chatflows-streaming/chat'],
        ['GET', '/api/v1/ping-extra'],
        ['GET', '/api/v1/ping/nested'],
        ['POST', '/api/v1/internal-prediction/chat'],
        ['DELETE', '/api/v1/chatflows/apikey/token'],
        ['GET', '/api/v1/PING']
